...
Shell equivalent (from repo root; replace SOURCE with your local UA assets tree):
  rsync -a --delete SOURCE/assets/sets/ resources/ua/bundled/sets/
  rsync -a --delete SOURCE/DATA/SCRIPTS/ resources/ua/bundled/shared_scripts/original/
  rsync -a --delete SOURCE/XP/DATA/SCRIPTS/ resources/ua/bundled/shared_scripts/metropolis_dawn/

After that, run tools/ua_reencode_text_to_utf8.py on resources/ua/bundled if Godot logs UTF-8 warnings.

Runtime resolution (map/ua_project_data_roots.gd): uses resources/ua/bundled for sets and shared scripts.
