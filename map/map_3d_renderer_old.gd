...
func build_from_current_map() -> void:
	...
	var result := build_mesh_with_textures(hgt, typ, w, h, mapping, subsector_patterns, tile_mapping, pre.tile_remap if pre else {}, pre.subsector_idx_remap if pre else {}, current_set)
...
static func _decode_raw_to_fcv(raw_val: int, default_file: int) -> Array:
	...
	var cells: int
	var v: int
	# UA universal mapping (from reversing):
	#  0-3   -> default surface file (ground_{surface}), 2x2, variants 0..3
	#  4-7   -> file 1, 2x2, variants 0..3
	#  8-11  -> file 2, 2x2, variants 0..3
	#  12-15 -> file 3, 2x2, variants 0..3
	#  16-31 -> file 4, 4x4, variants 0..15
	#  32-35 -> file 5, 2x2, variants 0..3
	#  36-127-> cycle files 0..5 (variant 0)
	#  128+  -> fallback to default file (variant 0)
	var n := raw_val
	if n < 0:
		n = 0
	if n <= 3:
...
static func _remap_subsector_idx(si: int, remap_table: Dictionary) -> int:
	if remap_table and remap_table.has(str(si)):
		return int(remap_table[str(si)])
	elif remap_table.has(si):
		return int(remap_table[si])
	return si

static func _incr_count(counts: Dictionary, key: int) -> void:
	counts[key] = int(counts.get(key, 0)) + 1

static func _draw_quad(st: SurfaceTool, xl: float, xr: float, zt: float, zb: float, y: float, f: int, cells: int, v: int, rot_deg: int, u0: float, vv0: float, u1: float, vv1: float) -> void:
	...
	st.set_uv(uv_nw); st.add_vertex(Vector3(xl, y, zt))
	st.set_uv(uv_ne); st.add_vertex(Vector3(xr, y, zt))
	st.set_uv(uv_se); st.add_vertex(Vector3(xr, y, zb))
	st.set_uv(uv_nw); st.add_vertex(Vector3(xl, y, zt))
	st.set_uv(uv_se); st.add_vertex(Vector3(xr, y, zb))
	st.set_uv(uv_sw); st.add_vertex(Vector3(xl, y, zb))

static func _cell_rect(ix0: float, _ix1: float, iz0: float, _iz1: float, step_x: float, step_z: float, cx_i: int, cy_i: int) -> Array:
	var xl := ix0 + float(cx_i) * step_x
	var xr := xl + step_x
	var zt := iz0 + float(cy_i) * step_z
	var zb := zt + step_z
	return [xl, xr, zt, zb]
...
static func build_mesh_with_textures(hgt: PackedByteArray, typ: PackedByteArray, w: int, h: int, mapping: Dictionary, subsector_patterns: Dictionary = {}, tile_mapping: Dictionary = {}, tile_remap: Dictionary = {}, subsector_idx_remap: Dictionary = {}, set_id: int = 1) -> Dictionary:
	var bw := w + 2
	var bh := h + 2
	...
	# Missing tilemap debug counter
	var _missing_tilemap := {}
	...
	for y in bh:
		for x in bw:
			...
			if override_typ0:
				...
				composed = true
			if not composed and not is_border and set_id == 1 and typ_value == 185:
				# Manual override for set1 typ185 top composition (approximation to aid debugging)
				var step_xm: float = (ix1 - ix0) / 3.0
				var step_zm: float = (iz1 - iz0) / 3.0
				# Row 0: NW, N, NE
				var r := _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 0, 0); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 2, 4, 0, 0, 0.0, 0.0, 1.0, 1.0) # ground_2 var0
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 1, 0); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 2, 4, 0, 0, 0.0, 0.25, 1.0, 0.75) # ground_2 var0 (offset)
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 2, 0); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 2, 4, 2, 0, 0.0, 0.0, 1.0, 1.0) # ground_2 var2
				# Row 1: W, C, E
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 0, 1); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 1, 4, 2, 90, 0.0, 0.0, 1.0, 1.0) # ground_1 var2 rotated -90
				# Center split: top half ground_2 var0, bottom half ground_4 var10 rotated 90
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 1, 1)
				var xl_c: float = float(r[0]); var xr_c: float = float(r[1]); var zt_c: float = float(r[2]); var zb_c: float = float(r[3]); var zm_c: float = (zt_c + zb_c) * 0.5
				_draw_quad(st, xl_c, xr_c, zt_c, zm_c, y_c, 2, 4, 0, 0, 0.0, 0.0, 1.0, 0.5)
				var xm_cb: float = (xl_c + xr_c) * 0.5
				_draw_quad(st, xl_c, xm_cb, zm_c, zb_c, y_c, 4, 16, 13, 270, 0.0, 1.0, 1.0, 0.0)
				_draw_quad(st, xm_cb, xr_c, zm_c, zb_c, y_c, 4, 16, 9, 270, 0.0, 0.0, 1.0, 1.0)
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 2, 1); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 1, 4, 2, 90, 0.0, 0.0, 1.0, 1.0) # ground_1 var2 rotated -90
				# Row 2: SW, S (composite), SE
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 0, 2); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 2, 4, 2, 0, 0.0, 0.0, 1.0, 1.0) # ground_2 var2
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 1, 2)
				var xm_s: float = (float(r[0]) + float(r[1])) * 0.5
				_draw_quad(st, r[0], xm_s, r[2], r[3], y_c, 1, 4, 2, 0, 0.0, 0.0, 0.5, 1.0) # left half: ground_1 var2, 4x2 subimage (x:1,y:2)
				_draw_quad(st, xm_s, r[1], r[2], r[3], y_c, 2, 4, 0, 0, 0.25, 0.0, 0.5, 0.5) # right half: ground_2 var0, 4x2 subimage (x:2,y:1)
				r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 2, 2); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 2, 4, 0, 0, 0.0, 0.0, 1.0, 1.0) # ground_2 var0
				composed = true
				...
				if sector_t == 0 and subs.size() >= 9:
					...
					for cy in 3:
						for cx in 3:
							...
							var si_rem := _remap_subsector_idx(si, subsector_idx_remap)
							if not tile_mapping.has(si_rem):
								_incr_count(_missing_tilemap, si)
							var td: Dictionary = tile_mapping.get(si_rem, {"val0": 0, "val1": 0, "val2": 0, "val3": 0, "flag": 0})
						...
						# end mosaic cell
						composed = true
				elif set_id == 1 and typ_value == 185:
					# Manual override for set1 typ185 top composition (approximation to aid debugging)
					var step_xm: float = (ix1 - ix0) / 3.0
					var step_zm: float = (iz1 - iz0) / 3.0
					# Row 0: NW, N, NE
					...
					var xl_c: float = float(r[0]); var xr_c: float = float(r[1]); var zt_c: float = float(r[2]); var zb_c: float = float(r[3]); var zm_c: float = (zt_c + zb_c) * 0.5
					_draw_quad(st, xl_c, xr_c, zt_c, zm_c, y_c, 2, 4, 0, 0, 0.0, 0.0, 1.0, 0.5)
					var xm_cb2: float = (xl_c + xr_c) * 0.5
					_draw_quad(st, xl_c, xm_cb2, zm_c, zb_c, y_c, 4, 16, 13, 270, 0.0, 1.0, 1.0, 0.0)
					_draw_quad(st, xm_cb2, xr_c, zm_c, zb_c, y_c, 4, 16, 9, 270, 0.0, 0.0, 1.0, 1.0)
					r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 2, 1); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 1, 4, 2, 90, 0.0, 0.0, 1.0, 1.0) # ground_1 var2 rotated -90
					# Row 2: SW, S (composite), SE
					r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 0, 2); _draw_quad(st, r[0], r[1], r[2], r[3], y_c, 2, 4, 2, 0, 0.0, 0.0, 1.0, 1.0) # ground_2 var2
					r = _cell_rect(ix0, ix1, iz0, iz1, step_xm, step_zm, 1, 2);
					...
					for cy1 in 3:
						for cx1 in 3:
							# Sector coords in typ space for world-subcell addressing
							var si1: int = subs[0]
							var si1_rem := _remap_subsector_idx(si1, subsector_idx_remap)
							if not tile_mapping.has(si1_rem):
								_incr_count(_missing_tilemap, si1)
							var td1: Dictionary = tile_mapping.get(si1_rem, {"val0": 0, "val1": 0, "val2": 0, "val3": 0, "flag": 0})
					...
					for cy2 in 2:
						for cx2 in 2:
							...
							var si2_rem := _remap_subsector_idx(si2, subsector_idx_remap)
							if not tile_mapping.has(si2_rem):
								_incr_count(_missing_tilemap, si2)
							var td2: Dictionary = tile_mapping.get(si2_rem, {"val0": 0, "val1": 0, "val2": 0, "val3": 0, "flag": 0})
			...
			if override_typ0:
				...
				if int(patE.get("sector_type", -1)) == 0:
					...
					if subsE.size() >= 9:
						...
						var idxN_rem := _remap_subsector_idx(idxN, subsector_idx_remap)
						if not tile_mapping.has(idxN_rem):
							_incr_count(_missing_tilemap, idxN)
						var tN: Dictionary = tile_mapping.get(idxN_rem, {"val0": 0, "val1": 0, "val2": 0, "val3": 0, "flag": 0})
						...
						var idxS_rem := _remap_subsector_idx(idxS, subsector_idx_remap)
						if not tile_mapping.has(idxS_rem):
							_incr_count(_missing_tilemap, idxS)
						var tS: Dictionary = tile_mapping.get(idxS_rem, {"val0": 0, "val1": 0, "val2": 0, "val3": 0, "flag": 0})
						...
						var idxW_rem := _remap_subsector_idx(idxW, subsector_idx_remap)
						if not tile_mapping.has(idxW_rem):
							_incr_count(_missing_tilemap, idxW)
						var tW: Dictionary = tile_mapping.get(idxW_rem, {"val0": 0, "val1": 0, "val2": 0, "val3": 0, "flag": 0})
						...
						var idxE_rem := _remap_subsector_idx(idxE, subsector_idx_remap)
						if not tile_mapping.has(idxE_rem):
							_incr_count(_missing_tilemap, idxE)
						var tE: Dictionary = tile_mapping.get(idxE_rem, {"val0": 0, "val1": 0, "val2": 0, "val3": 0, "flag": 0})
	...
	# Debug print for missing indices after commit
	if not _missing_tilemap.is_empty():
		print("[Map3D] missing tile_mapping after remap (counts): ", _missing_tilemap)
