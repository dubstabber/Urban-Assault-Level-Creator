...
func test_hgt_border_indices_expand_to_localized_sectors_and_chunks() -> bool:
	...
	_check(chunks.has(TerrainBuilder.sector_to_chunk(0, 0)), "Expected top-left chunk to be marked dirty for top-left border edit")
...
func test_typ_indices_mark_effective_typ_dirty_and_primary_chunk() -> bool:
	...
	_check(chunks.has(TerrainBuilder.sector_to_chunk(3, 4)), "Expected edited typ sector chunk to be tracked")
	return _errors.is_empty()
...
func test_blg_indices_use_primary_chunk_neighborhood_only() -> bool:
	...
	_check(chunks.has(TerrainBuilder.sector_to_chunk(6, 1)), "Expected primary chunk for blg edit to be included")
	return _errors.is_empty()
