...
func _update_properties() -> void:
	if EditorState.selected_unit is Squad:
		if Preloads.squad_images.has(int(EditorState.selected_unit.vehicle)):
			%SquadIcon.texture = Preloads.squad_images[int(EditorState.selected_unit.vehicle)]
		...
		%XposSquadLineEdit.text = str(roundi(EditorState.selected_unit.position.x))
		%ZposSquadLineEdit.text = str(roundi(-EditorState.selected_unit.position.y))
