...
func _ready() -> void:
	%NormalKeySectorOptionButton.item_selected.connect(func(index: int):
		if %NormalKeySectorOptionButton.get_item_id(index) < 256:
			CurrentMapData.typ_map[CurrentMapData.selected_sector_idx] = %NormalKeySectorOptionButton.get_item_id(index)
	...
	%ParasiteKeySectorOptionButton.item_selected.connect(func(index: int):
		if %ParasiteKeySectorOptionButton.get_item_id(index) < 256:
			CurrentMapData.typ_map[CurrentMapData.selected_sector_idx] = %ParasiteKeySectorOptionButton.get_item_id(index)
			EventSystem.map_updated.emit()
		)
...
func set_labels(key_sector_id: int, bomb_id: int, bomb_building_type: String) -> void:
	...
	match bomb_building_type:
		"normal": 
			...
			var item_index = %NormalKeySectorOptionButton.get_item_index(CurrentMapData.typ_map[CurrentMapData.selected_sector_idx])
			if item_index >= 0:
		...
		"parasite":
			...
			var item_index = %ParasiteKeySectorOptionButton.get_item_index(CurrentMapData.typ_map[CurrentMapData.selected_sector_idx])
			if item_index >= 0:
