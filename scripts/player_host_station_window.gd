...
func _on_about_to_popup() -> void:
	if CurrentMapData.host_stations.get_child_count() <= 0:
		...
		if CurrentMapData.player_host_station >= CurrentMapData.host_stations.get_child_count():
			CurrentMapData.player_host_station = 0
		...
		for i in CurrentMapData.host_stations.get_child_count():
			...
			if CurrentMapData.player_host_station == i:
